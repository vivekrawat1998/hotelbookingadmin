...
import { deleteServiceThunk, getServiceThunk, resetState } from "../../features/services/serSlice";
...
const Service = () => {
  ...
  const visMisState = useSelector((state) => state?.service?.service);
  ...
  for (let i = 0; i < visMisState?.length; i++) {
    data1?.push({
      ...
      action: (
        <>
          <Link
            to={`/admin/add-service/${visMisState[i]?._id}`}
  ...
  return (
    <>
    <div>
      <h3 className="mb-4 title"> Service Data</h3>
      <div>
